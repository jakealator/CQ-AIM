...
function nearFieldMatrix = assembleNearFieldMatrices(triAreas,nearFieldDistances, iElements, jElements, centroids, c,c0,s,N)

% Contrasts at centroid points
qj =((c0/c(centroids(:,1),centroids(:,2)))^2-1);
...
for k = 1:length(iElements)
    if iElements(k)==jElements(k) % On the diagonal
        KElements(k) = -triAreas(iElements(k)).*(4*1i*c0^2/s^2+(2*pi*1i*c0/s).*...
